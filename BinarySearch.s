...
msg:		.asciiz	" "
string_Done: .asciiz "\nBubbleSort Done!"
inputsequential: .asciiz "\nInput Sequential : "
showsequential: .asciiz "\nSequential is Found"
notshowsequential: .asciiz "\nSequential is Not found"
.text
main:
...
sequentialsearch:	
        ...
        la      $a0, inputbinary 	    # print input sequential
        syscall   
		li 		$v0, 5                  # read integer
		syscall
		move 	$t5, $v0	
		la 		$a0,number				# load address number to a0
inloopsequentialsearch:
		
		bge 	$t0,$s6,exit3			#if i >= n exit loop
		mul     $t4,$t0,4               #$t4 = i * 4
		add     $t3,$t4,$a0				#$t3 = &number[i]
		lw      $t4,0($t3)				#$t4 = number[i]
		beq     $t5,$t4,exit2			#$t4 = $t5 go to exit2
		addi 	$t0, $t0, 1 			#i++
		j       inloopsequentialsearch		
exit2:
		li 		$v0, 4
		la      $a0, foundsequential 
		syscall
		j 		endSort
exit3:
		li 		$v0, 4
		la      $a0, notfoundsequential 
		syscall


binarysearch:
		li   	$v0,4              		# $v0 = 4 print string
        la      $a0, inputbinary	    # print input binary
        syscall   
		li 		$v0, 5                  # read integer
		syscall
		move 	$t5, $v0	
		...
		li		$t0,0					#i=0
		li		$s0,0					#first = 0
		addi	$s1,$s6,-1              #last = s6 - 1 = (n-1)
		add     $s2,$s0,$s1             #middle = first + last
		div     $s2, $t6                #$s2/2
		mflo	$s3						#$s3 = div = (first+last)/2
loopbinarysearch:
		bgt		$s0,$s1,exit4			#fisrt > last go to exit4
		mul     $s4,$s3,4               #$s4 * 4 = middle * 4 
		add     $s5,$s4,$a0             #$s5 = &number[middle]
		lw		$s4,0($s5)				#$s4 = number[middle]
		bgt		$t5,$s4,plusfirst		#if search > number[middle] go to plusfirst
		beq		$s4,$t5,foundbinarysearch
		addi	$s1,$s2,-1              #last = middle - 1
		add     $s2,$s0,$s1             #middle = first + last
		div     $s2, $t6                #$s1/2
		mflo	$s3						#$s3 = div = (first+last)/2
		j       loopbinarysearch
plusfirst:
		addi	$s0,$s3,1
		add  	$s3,$s0,$s1             #$s3 = first + last
		div     $s3,$t6                 #$s3 = (first + last)/2
		mflo    $s3						#$s3 = (first + last)/2
		j       loopbinarysearch
foundbinarysearch:
		li   	$v0,4              		# $v0 = 4 print string
        la      $a0,foundbinary         # print input binary
        syscall         
        j       endSort
exit4:
		li   	$v0,4              		# $v0 = 4 print string
        la      $a0,notfoundbinary      # print input sequential
        syscall         
endSort1:
